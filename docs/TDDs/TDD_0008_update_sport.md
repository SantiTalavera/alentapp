...
Ya creado el deport en el sistema, permitir que un administrativo modifique los datos editables de un deporte ya registrado, manteniendo actualizada la información del catálogo deportivo sin comprometer las reglas de negocio definidas para la entidad.
...
- Si el deporte no existe, la operación debe fallar con un error claro.
- Si el body no contiene campos modificables, la operación debe fallar con un error de validación.
- Al completarse correctamente, el sistema debe devolver el deporte con todos sus datos actualizados.
...
3. **Caso de Uso**: `UpdateSportUseCase` (Orquesta la operación). Recibe el `id` y el body del request, verifica que el deporte exista, valida los campos enviados, rechaza cualquier intento de modificar `name` y llama al repositorio para persistir los cambios.
...
5. **Adaptador de Entrada**: `SportController` (Ruta HTTP `PATCH /api/v1/deportes/:id`). Extrae el `id` de la URL, valida el body tipado como `UpdateSportRequest`, invoca el caso de uso y mapea excepciones a códigos HTTP.
...
2. Agregar el método `update(id, data)` al puerto `SportRepository` y asegurar que exista un método `findById(id)` para validar la existencia del deporte.
...
4. Implementar `UpdateSportUseCase`, verificando existencia del deporte, body no vacío, campos permitidos y reglas de negocio antes de persistir.
5. Implementar el método `update` en `PostgresSportRepository` usando Prisma.
6. Implementar el endpoint `PATCH /api/v1/deportes/:id` en `SportController` y registrarlo en Fastify.
7. Crear o reutilizar el formulario/modal de edición en React, deshabilitando o excluyendo el campo `name`.
8. Escribir tests unitarios para el caso de uso: deporte inexistente, intento de modificar `name`, `max_capacity` inválido, `additional_price` inválido, body vacío y actualización exitosa.
